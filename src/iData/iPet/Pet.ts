	...
	export module iPet {
		export class Pet {
			...
			public addExp(exp: number): any {
				...
				iPanel.iScene.MainScene.allInfoPanel.addText("你的宠物获得了<font color=\'#4a60d7\'>" + exp + "</font>经验.", iGlobal.Global.exp);
			...
			private levelUp(): any {
				...
				iPanel.iScene.MainScene.allInfoPanel.addText("<font color=\'#ff4040\'>你的宠物升级了!你的宠物达到了Lv." + this.level + "!</font>");
				if (Math.random() * 100 < 1 - this.level * 0.01) {
					...
					if (this.addSkill(iData.iPet.iPetSkill.PetSkillList.list[_loc3_])) {
						iPanel.iScene.MainScene.allInfoPanel.addText("<font color=\'#ff4040\'>你的宠物学会了" + iData.iPet.iPetSkill.PetSkillList.list[_loc3_].name + "!</font>");
					}
				}
			}
			...
			public set level(level: number) {
				...
				if (iPanel.iScene.MainScene.otherPanel) {
					if (iPanel.iScene.MainScene.otherPanel.equipWindow) {
						iPanel.iScene.MainScene.otherPanel.equipWindow.updatePetInfo();
					}
				}
			}
			...
			public set exp(param1: number) {
				...
				if (iPanel.iScene.MainScene.petInfoPanel) {
					iPanel.iScene.MainScene.petInfoPanel.updateExp();
				}
			}
